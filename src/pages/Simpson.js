...
const Simpson = () => {
  ...
  const [n, setN] = useState(4); // n debe ser par
  ...
  const handleSubmit = async (e) => {
    ...
    if (Number(n) < 2 || Number(n) % 2 !== 0) {
      setErrorMessage('n debe ser un entero par mayor o igual a 2.');
      return;
    }

    const backendUrl = `http://localhost:5008/simpson?ecuacion=${encodeURIComponent(ecuacion.replace(/\s+/g, ''))}&a=${a}&b=${b}&n=${n}`;
    try {
  ...
  const plotData = () => {
    ...
    // Trazas de los segmentos de Simpson (para visualización similar a trapecios)
    const segmentos = results.xi.slice(0, -1).map((xi, i) => ({
      x: [results.xi[i], results.xi[i], results.xi[i + 1], results.xi[i + 1]],
      y: [0, results.fxi[i], results.fxi[i + 1], 0],
      fill: 'toself',
      type: 'scatter',
      mode: 'lines',
      line: { color: 'rgba(0,200,100,0.3)' },
    ...
    return (
      <Plot
        data={[functionTrace, ...segmentos]}
        layout={{
          title: 'Aproximación por el Método de Simpson',
          xaxis: { title: 'x' },
          yaxis: { title: 'f(x)' },
          legend: { x: 1, y: 1 },
        }}
  ...
  return (
    <div className="home-container">
      <div className="api-container2">
        <h1 className="home-title">Método de Simpson 1/3</h1>
        <form onSubmit={handleSubmit} className="biseccion-form">
          ...
          <div className="form-group">
            <label>Número de subintervalos (n, par):</label>
            <input type="number" value={n} onChange={e => setN(e.target.value)} min="2" step="2" />
          </div>
        ...
        {results && (
          <div className="results-table">
            ...
            <table>
              <tbody>
                <tr>
                  <th>Integral aproximada (Simpson)</th>
                  <td>{results.integral_simpson}</td>
                </tr>
                <tr>
            ...
            <div className="plot-container">
              <h2>Gráfica del Método de Simpson</h2>
              {plotData()}
            </div>
          </div>
        )}
